...
export default function App() {
  ...
  // ⚡ Bolt: wrap deleteLocal in useCallback to prevent breaking memoization
  ...
  // ⚡ Bolt: replace array spread with sequential some() to prevent GC overhead during frequent render loops
  const isLandmarkCollected = useCallback((name: string, lat: number, lng: number) => {
    const isMatch = (l: { name: string; lat: number; lng: number }) => l.name === name || (Math.abs(l.lat - lat) < 0.0001 && Math.abs(l.lng - lng) < 0.0001);
    return collectedLandmarks.some(isMatch) || localLandmarks.some(isMatch);
  }, [collectedLandmarks, localLandmarks]);

  // ⚡ Bolt: wrap deleteCollected in useCallback to prevent breaking memoization
  ...
  // ⚡ Bolt: memoize allLandmarks and handleDelete to avoid allocating new arrays/functions on every render
  const allLandmarks = useMemo(() => [...collectedLandmarks, ...localLandmarks], [collectedLandmarks, localLandmarks]);
  const handleDelete = useCallback((id: string) => id.startsWith('local_') ? deleteLocal(id) : deleteCollected(id), [deleteLocal, deleteCollected]);
