        ...
        {error && (
          ...
          >
            <div className="bg-brand-bg/90 backdrop-blur-xl border-2 border-red-500/50 text-red-400 p-4 rounded-2xl shadow-2xl flex items-start gap-3">
              ...
              <button onClick={() => setError(null)} aria-label="Dismiss alert" className="p-1 hover:bg-white/5 rounded-lg transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
          </motion.div>
              ...
              <div className="relative">
                {!image ? (
                  ...
                  <div className="relative aspect-[4/3] rounded-[40px] overflow-hidden shadow-2xl border-4 border-white/10">
                    ...
                    <button onClick={() => setImage(null)} aria-label="Remove photo" className="absolute top-6 right-6 p-3 bg-black/50 backdrop-blur-md rounded-full text-white hover:bg-black/70 transition-colors z-30"><X className="w-5 h-5" /></button>
                    {!result && !isAnalyzing && (
