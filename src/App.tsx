...
export default function App() {
  ...
  // ⚡ Bolt: Memoize callback and prevent intermediate array allocation during high-frequency heading updates
  const isLandmarkCollected = useCallback((name: string, lat: number, lng: number) => {
    const check = (l: CollectedLandmark) => l.name === name || (Math.abs(l.lat - lat) < 0.0001 && Math.abs(l.lng - lng) < 0.0001);
  ...
  return (
    <div className="min-h-screen flex flex-col scanline">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-10 pb-32">
        {showChronicle ? (
          <FeedSystem 
            landmarks={React.useMemo(() => [...collectedLandmarks, ...localLandmarks], [collectedLandmarks, localLandmarks])}
