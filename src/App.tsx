...
export default function App() {
  ...
  // ⚡ Bolt: Prevent array allocation overhead in high-frequency render path
  // Sequential evaluation avoids creating a new array on every device heading update
  const isLandmarkCollected = useCallback((name: string, lat: number, lng: number) => {
    const isCollected = (l: CollectedLandmark) => l.name === name || (Math.abs(l.lat - lat) < 0.0001 && Math.abs(l.lng - lng) < 0.0001);
    return collectedLandmarks.some(isCollected) || localLandmarks.some(isCollected);
