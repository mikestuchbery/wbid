...
export default function App() {
  ...
  const isLandmarkCollected = useCallback((name: string, lat: number, lng: number) => {
    // ⚡ Bolt: Prevent intermediate array allocation and GC overhead in hot render loops
    // by avoiding array spreads [...a, ...b] and using sequential evaluations instead.
    const checkMatch = (l: { name: string; lat: number; lng: number }) =>
      l.name === name || (Math.abs(l.lat - lat) < 0.0001 && Math.abs(l.lng - lng) < 0.0001);
    return collectedLandmarks.some(checkMatch) || localLandmarks.some(checkMatch);
