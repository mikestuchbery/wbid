...
export default function App() {
  ...
  // ⚡ Bolt Performance Optimization: Prevent array allocations in hot render paths
  // Replaced [...a, ...b].some(...) with a.some(...) || b.some(...) to avoid GC pressure
  // Memoized with useCallback to preserve reference stability during high-frequency heading updates
  const isLandmarkCollected = useCallback((name: string, lat: number, lng: number) => {
    const check = (l: { name: string; lat: number; lng: number }) =>
