...
export default function App() {
  ...
  // ⚡ Bolt: Prevent array allocation and garbage collection in hot render loops
  // by avoiding the spread operator for merging arrays, and memoize with useCallback.
  const isLandmarkCollected = useCallback((name: string, lat: number, lng: number) => {
    const isFound = (l: { name: string; lat: number; lng: number }) =>
      l.name === name || (Math.abs(l.lat - lat) < 0.0001 && Math.abs(l.lng - lng) < 0.0001);
    return collectedLandmarks.some(isFound) || localLandmarks.some(isFound);
