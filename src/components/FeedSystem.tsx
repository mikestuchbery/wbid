...
export const FeedSystem: React.FC<FeedSystemProps> = ({ 
  ...
  // ⚡ Bolt: Memoize sorted list and distance calculations to prevent O(N log N)
  // sorting and expensive Haversine math on every render (e.g., from deviceorientation).
  // Split sorting and distance mapping into separate memos to avoid re-sorting when
  // only userLocation changes.
  ...
  const processedLandmarks = React.useMemo(() => {
    return sortedLandmarks.map(lm => {
      ...
      if (userLocation) {
        ...
        const dist = R * c;
        distanceString = dist < 1 ? `${(dist * 1000).toFixed(0)}m` : `${dist.toFixed(1)}km`;
      }
      return { ...lm, distanceString };
    });
  }, [sortedLandmarks, userLocation]);
