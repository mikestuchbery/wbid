...
export const FeedSystem: React.FC<FeedSystemProps> = ({ 
  ...
  // Memoize sorted landmarks to avoid O(N log N) sorting on every render
  ...
  // Memoize expensive Haversine distance calculations
  const distances = useMemo(() => {
    ...
    landmarks.forEach(lm => {
      ...
      const dist = R * c;
      cache[lm.id] = dist < 1 ? `${(dist * 1000).toFixed(0)}m` : `${dist.toFixed(1)}km`;
    });
