...
import { cn, calculateDistance, formatDistance } from '../utils';
...
interface ProcessedLandmark extends CollectedLandmark {
...
export const FeedSystem: React.FC<FeedSystemProps> = ({ 
  ...
  const processedLandmarks = useMemo<ProcessedLandmark[]>(() => {
    return [...landmarks]
      .sort((a, b) => {
      ...
      .map(lm => {
        ...
        if (userLocation) {
          const dist = calculateDistance(lm.lat, lm.lng, userLocation.lat, userLocation.lng);
          distanceString = formatDistance(dist);
        }
        return { ...lm, distanceString };
      });
              ...
              >
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-6 right-6 flex gap-2">
                    <button 
                      onClick={() => {
                        if (window.confirm('Are you sure you want to delete this historical discovery? This action cannot be undone.')) {
                          onDelete(lm.id);
                        }
                      }}
                      className="p-3 bg-black/40 backdrop-blur-md rounded-full text-red-400 hover:bg-red-500 hover:text-white transition-all active:scale-90 border border-white/10 focus-visible:ring-2 focus-visible:outline-none"
                      title="Delete Entry"
