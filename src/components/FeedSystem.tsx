...
export const FeedSystem = React.memo(({
  ...
  // ⚡ Bolt: Memoize the sorted landmarks array to prevent O(N log N) recalculations
  // on every render, especially during high-frequency device orientation updates.
  // Impact: Reduces CPU time spent on sorting when the feed is rendered.
                ...
                <div className="p-8 space-y-6">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[10px] font-mono opacity-40 uppercase tracking-widest">
                      ...
                      <DistanceIndicator lm={lm} userLocation={userLocation} />
                    </div>
                  </div>
...
// ⚡ Bolt: Extract distance calculation into a memoized sub-component to prevent
// inline function instantiation and complex math execution on every render cycle.
// Impact: Significantly reduces main thread blocking during frequent parent re-renders.
const DistanceIndicator = React.memo(({ lm, userLocation }: { lm: CollectedLandmark, userLocation: { lat: number, lng: number } | null }) => {
